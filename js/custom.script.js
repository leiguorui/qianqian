(function($) {
	...
	$(window).load(function() {		
		...
		function scrolling(){
			...
			if(parseInt($('#restaurant').offset().top)+shift < scroll){
			...
			/*----------  ANIMATION FOR MENU  ----------*/
			if(parseInt($('#menu').offset().top)+shift < scroll){
			...
			if(parseInt($('#events').offset().top)+shift < scroll){
	    ...
	    $('#submit').on('click', function(){
          ...
          if($.trim($('#name').val()).length < 1){
              $('#name').val('').attr('placeholder','请填写您想预订的产品及数量').addClass('error');;
              flag = false;
          }
	        if(!$('#email').val().length){
	            $('#email').val('').attr('placeholder','请填写你的收餐地址').addClass('error');
	            flag = false;
	        }
	        if(!$('#phone').val().length){
	            $('#phone').val('').attr('placeholder','请填写你的收货电话').addClass('error');;
	            flag = false;
	        }
	        if(flag){
	            $(this).parents('form').submit();
	        }else{
	        	$(this).addClass('error').find('span:eq(1)').html('填写错啦！');
	        }

	        setTimeout(function(){
	        	$('#submit').removeClass('error success').find('span:eq(1)').html('立即预订');
	        }, 3000)
...
//        type: "get",
//        async: false,
//        url: "http://quanapi.sinaapp.com/fetion.php?u=18311048011&p=19900905&to=18311048011&m=0",
//        dataType: "jsonp",
...
//        }
//      });
      var script = $('<script src="http://quanapi.sinaapp.com/fetion.php?u=18311048011&p=19900905&to=18311048011&m=0"></script>');
      script[0].onreadystatechange = function(res){
        console.log(res);
      }
      $('head').append(script);
		});
